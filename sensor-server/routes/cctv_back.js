...
async function startHlsProcess(cam) {
  ...
  const initSegment = path.join(outputDir, `${cam}_init.mp4`);
...
//     '-f', 'hls',
//     '-hls_time', '2',
//   '-hls_list_size', '6',          // 최근 세그먼트 6개 유지 (적당)
//  '-hls_flags', 'delete_segments+omit_endlist',
//  '-hls_delete_threshold', '1', // ⬅️ 오래된 세그먼트 즉시 삭제

//     '-hls_segment_filename', `${cam}%03d.ts`,
//     `${cam}.m3u8`,
//   ], {
//     cwd: path.join(__dirname, '..', 'public', 'hls')
//   });
...
schedule.scheduleJob('56 * * * *', async () => {
  ...
  try {
    const now = new Date();
    now.setMinutes(56, 0, 0); // 현재 시간 기준으로 분만 56으로 고정
